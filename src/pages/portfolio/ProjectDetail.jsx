...
const ProjectDetail = () => {
  ...
  useEffect(() => {
    ...
    if (foundProject && foundProject.category) {
      const categoryProjects = categories[foundProject.category] || [];
      const related = categoryProjects
        .filter((p) => {
          const pId = p.id
            ? `${foundProject.category}-${p.id}`
            : `${foundProject.category}-${p.title.replace(/\s+/g, "-")}`;
                ...
                {relatedProjects.map((proj) => {
                  ...
                  return (
                    <Link
                      ...
                      to={`/portfolio/${formatCategoryForUrl(proj.category)}/${
                        proj.compositeId
                      }`}
